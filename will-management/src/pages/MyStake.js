...
function MyStake() {
  ...
  const handleOpenModal = (assetCategory, amount, createdAt, lastModified) => {
    setSelectedWillDetails({ assetCategory, amount, createdAt, lastModified });
    setIsModalOpen(true);
  };
  ...
  const generatePDF = (will) => {
    // If you want to add an image, set its base64 data (replace with actual image base64 if needed)
    const tickBase64 =
    ...
    if (!will) {
      console.error("Will data is not available");
      return;
    }
    ...
    // Asset Category
    if (will.assetCategory) {
      doc.setFillColor(...sectionColors.default);
      doc.rect(10, yPosition, 190, 10, "F");
      doc.setTextColor(0);
      doc.text(`Asset Category: ${will.assetCategory}`, 12, yPosition + 7);
      yPosition += 15;
    }
    ...
    if (will.amount) {
      doc.setFillColor(...sectionColors.default);
      doc.rect(10, yPosition, 190, 10, "F");
      doc.text(`Amount (ETH): ${will.amount}`, 12, yPosition + 7);
      yPosition += 15;
    }
    ...
    if (will.createdAt) {
      doc.setFillColor(...sectionColors.default);
      doc.rect(10, yPosition, 190, 10, "F");
      doc.text(`Created At: ${will.createdAt}`, 12, yPosition + 7);
      yPosition += 15;
    }

    // Last Modified
    if (will.lastModified) {
      doc.setFillColor(...sectionColors.default);
      doc.rect(10, yPosition, 190, 10, "F");
      doc.text(`Last Modified: ${will.lastModified}`, 12, yPosition + 7);
      yPosition += 15;
    }
    ...
    doc.setFillColor(...sectionColors.default);
    doc.rect(10, yPosition, 190, 10, "F");
    doc.text(`Downloaded On: ${currentDateTime}`, 12, yPosition + 7);

    if (tickBase64) {
      doc.addImage(tickBase64, "PNG", 100, 200, 30, 30); // Adding image at 10,10 with size 50x50
      doc.setFontSize(12); // Set font size
      doc.setTextColor(0); // Set text color to black
      doc.text("Digitally Verified", 115, 235, { align: "center" });
    }

    // Save the PDF
    const fileName = will.assetCategory
      ? `${will.assetCategory}_Details.pdf`
      : "Will_Details.pdf";
    doc.save(fileName);
  };
      ...
      <table className="min-w-full table-auto border-collapse border border-gray-300">
        ...
        <tbody>
          {data.length === 0 ? (
            ...
            data
              .filter((row) => row.isVerified === true) // Only show rows where isVerified is true
              .map((row, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="px-4 py-2 border border-gray-300 text-center">
                    <button
                      ...
                      onClick={() =>
                        handleOpenModal(
                          row.assetCategory?.toString(),
                          ethers.utils.formatEther(row.amount || "0"),
                          new Date(
                            row.createdAt?.toNumber() * 1000
                          ).toLocaleString(),
                          new Date(
                            row.lastModified?.toNumber() * 1000
                          ).toLocaleString()
                        )
                      }
                    >
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            {selectedWillDetails ? (
              <div>
                <p>
                  ...
                  {selectedWillDetails.assetCategory}
                </p>
                <p>
                  <strong>Amount (ETH):</strong> {selectedWillDetails.amount}
                </p>
                <p>
                  <strong>Created At:</strong> {selectedWillDetails.createdAt}
                </p>
                <p>
                  <strong>Last Modified:</strong>{" "}
                  {selectedWillDetails.lastModified}
                </p>
              </div>
            ) : (
            ...
            <div className="mt-4">
              ...
              <button
                onClick={() => generatePDF(selectedWillDetails)}
