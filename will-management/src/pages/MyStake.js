...
function MyStake() {
  ...
  const generatePDF = (willDetails) => {
    ...
    if (tickBase64) {
      doc.addImage(tickBase64, "PNG", 10, 10, 50, 50); // Adding image at 10,10 with size 50x50
    }
            ...
            data
              ...
              .map((row, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border border-gray-300 text-center">
                    {new Date(row.createdAt?.toNumber() * 1000).toLocaleString()}
                  </td>
                  <td className="px-4 py-2 border border-gray-300 text-center">
                    {new Date(row.lastModified?.toNumber() * 1000).toLocaleString()}
                  </td>
                </tr>
              ))
          )}
